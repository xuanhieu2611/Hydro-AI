...
- **Payments:** **RevenueCat** (`react-native-purchases`), wrapped by `lib/billing/` — screens never import the SDK directly (same net rule as Supabase). Hard paywall: after onboarding + sign-in, the app is gated behind the `premium` entitlement (`app/paywall.tsx`). Setup: `docs/PAYMENTS_SETUP.md`.
...
> Paywall flags: real billing runs in `supabase` mode with RevenueCat keys set (`EXPO_PUBLIC_REVENUECAT_*`, see `docs/PAYMENTS_SETUP.md`); needs a fresh dev-client build (new native module). `mock` mode is always entitled (paywall bypassed) so UI iteration stays fast — set `EXPO_PUBLIC_FORCE_PAYWALL=1` (or use the Dev screen lock/unlock) to preview the paywall, where a "purchase" just flips an in-memory flag. The single source of truth for entitlement is `lib/billing/context.tsx` (`useBilling`), consumed by the gate in `app/_layout.tsx`.
